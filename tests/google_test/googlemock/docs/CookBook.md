...
class ScopedMockLog : public LogSink {
 public:
  ...
  virtual void send(LogSeverity severity, const char* full_filename,
    ...
    // log message.
    INFO(severity, full_filename, std::string(message, message_len));
  }

  // Implements the mock method:
  //
  //   void INFO(LogSeverity severity,
  //            const string& file_path,
  //            const string& message);
  MOCK_METHOD3(INFO, void(LogSeverity severity, const string& file_path,
                         const string& message));
};
```
...
```cpp
  ...
  EXPECT_CALL(log, INFO(WARNING, _, "File too large."))     // #1
      .Times(AnyNumber())
      .InSequence(s1, s2);
  EXPECT_CALL(log, INFO(WARNING, _, "Data set is empty."))  // #2
      .InSequence(s1);
  EXPECT_CALL(log, INFO(WARNING, _, "User not found."))     // #3
      .InSequence(s2);
```
...
using ::testing::_;
...
  EXPECT_CALL(log, INFO(WARNING, _, _));                  // #1
  EXPECT_CALL(log, INFO(WARNING, _, "File too large."));  // #2
```
