...
TEST(polygon_intersections, polygons_that_are_lines_intersections) {
    Polygon line1{{0,0}, {1,1}};
    Polygon line2{{1,0}, {0,1}};

    EXPECT_TRUE(line1.intersects(line2));
    EXPECT_TRUE(line1.intersects(line2));

    EXPECT_FALSE((Polygon{{0,0}, {0,1}}.intersects(Polygon{{1,0}, {1,1}})));
}
...
TEST(polygon_intersections, points_touching_but_segments_intersect) {
    Polygon polygon1{{0,0}, {5,5}, {0,10}},
            polygon2{{5,0}, {0,5}, {5,10}};
...
TEST(curve_intersections, not_intersecting) {
    EXPECT_EQ(curve_intersections(Bezier<Point>{{0, 0},
                                                {1, 1}},
                                  Bezier<Point>{{0, 1},
                                                {1, 2}}), 0);
}
...
TEST(curve_intersections, higher_order_curves) {
    Bezier<Point> bezier{ {0,0}, {5,5}, {0,10}};
    Bezier<Point> bezier2{{5,0}, {0,5}, {5,10}};

    EXPECT_EQ(curve_intersections(bezier, bezier2), 2);
